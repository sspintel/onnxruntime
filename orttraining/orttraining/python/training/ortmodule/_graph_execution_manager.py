...
class GraphExecutionManager(GraphExecutionInterface):
    def __init__(
        ...
        # The default is FLOOD_FILL, expand FP16 computation regions in the
        # graph using allowed opcodes for the given level.
        ...
        # whereas Level 2 perdetermined "FP16 safe" opcodes include opcodes
        # that perform computation using contrib ops, GeLU, Dropout,
        # LayerNormalization, etc.
        self._propagate_cast_ops_level = 1
        # List of opcodes to be considered safe to move before/after cast
        # operation if propagate_cast_ops_level is zero.
        self._propagate_cast_ops_allow = []
        # Whether allow fusion of layer norm subgraph if doing so will cause
        # modified precision.
        ...
        # To be instantiated in the concrete implementation of
        # GraphExecutionManager
        self._export_mode = None

        # Exporter can take extra arguments for ORTModule extensions
        # It cannot overlap with required/immutable arguments (validated in
        # runtime)
        ...
        # if we observe benefit of static shape, we can expose this flag to
        # user
        self._use_static_shape = False

        # flag to enable symbolic shape inference for dynamic shape inputs to
        # improve performance
        ...
        # TODO: remove after PyTorch ONNX exporter supports VAR_KEYWORD
        # parameters.
        ...
        # assign self._torch_alloc and self._torch_free if
        # self._use_external_gpu_allocator is True
    ...
    def _get_torch_gpu_allocator_function_addresses(self):
        if self._use_external_gpu_allocator and torch.cuda.is_available():
            # CPP extension to get torch GPU allocator's alloc and free
            # function addresses
    ...
    def _build_graph(self):
        ...
        # Map each input/initializer to its gradient index in the graph output,
        # or -1 is gradient is not required.
        self._gradient_map = []
    ...
    def _get_session_config(self):
        ...
        if self._device.type == "cuda":
            # Configure the InferenceSessions to use the specific GPU on which
            # the model is placed.
            ...
            if not self.is_rocm_pytorch:
                # Set Conv algo search mode to HEURISTIC, which is same as
                # PyTorch's default setting.
    ...
    def _export_model(self, *inputs, **kwargs):
        ...
        # Return True if the model needed to be exported, False if no export
        # was required.
        ...
        # or the user explicitly changed model parameters after the onnx
        # export.

        # Record random states here and restore later in case any of them gets changed during the export,
        # e.g., some sympy functions in symbolic_shape_infer will change
        # Python's random state.
