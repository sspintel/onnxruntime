...
def forward(self, *inputs, **kwargs):
        ...
        try:
            # Export model
            if (
                "IF exporting module to ONNX for first time == true"
            ):
                # Exporting module to ONNX for the first time
                exported_model = self._export_model(*inputs, **kwargs)
                ...
                self._inference_session = onnxruntime.InferenceSession(
                    exported_model, session_options, providers, provider_options
                )

            run_options = C.RunOptions()
            # pre-process inputs to make them compatible with onnxruntime
            outputs = self._inference_session.run(<inputs>, run_options)
            return _io.unflatten_user_output(self._module_output_schema, ExecutionAgentOutput(outputs))
        except Exception as e:
            # Catch-all
            raise e
