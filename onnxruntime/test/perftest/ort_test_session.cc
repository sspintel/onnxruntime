...
OnnxRuntimeTestSession::OnnxRuntimeTestSession(Ort::Env& env, std::random_device& rd,
  ...
  if (provider_name == onnxruntime::kDnnlExecutionProvider) {
    ...
    while (ss >> token) {
      ...
      if (key == "device_type") {
        std::set<std::string> ov_supported_device_types = {"CPU_FP32", "GPU_FP32", "GPU_FP16", "VAD-M_FP16", "MYRIAD_FP16", "VPUX_U8", "VAD-F_FP32"};
        if (ov_supported_device_types.find(value) != ov_supported_device_types.end()) {
          ...
          ORT_THROW("[ERROR] [OpenVINO] You have selcted wrong configuration value for the key 'device_type'. select from 'CPU_FP32', 'GPU_FP32', 'GPU_FP16', 'VAD-M_FP16', 'MYRIAD_FP16', 'VPUX_U8', 'VAD-F_FP32' or from HETERO/MULTI/AUTO options available. \n");
        }
