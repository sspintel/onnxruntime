...
std::vector<NodeIndex> DataOps::GetUnsupportedNodeIndices(std::unordered_set<std::string>& ng_required_initializers, ias_handle_t sdk_handle) {
    ...
    for (const auto& node_idx : graph_viewer_.GetNodesInTopologicalOrder()) {
      ...
      if (node_is_supported(ng_supported_ops, node_idx)) {
        ...
        std::wstring wop_type = std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(optype);
        auto res = RecordEventEx(sdk_handle, nullptr, wop_type.c_str(), 1, 1.0, ekeys, evals, countof(ekeys));
        std::wcout << "Event recorded " << wop_type.c_str() << std::endl;
        ...
        ntype.append(optype);
        ntype.append(fback);
        std::wstring wop_type = std::wstring_convert<std::codecvt_utf8<wchar_t>>().from_bytes(ntype);
        auto res = RecordEventEx(sdk_handle, nullptr, wop_type.c_str(), 1, 1.0, ekeys, evals, countof(ekeys));
        std::wcout << "Event recorded " << wop_type.c_str() << std::endl;
        std::cout << "Res: " << res << std::endl;
        FAIL_ON_ERR(res);
