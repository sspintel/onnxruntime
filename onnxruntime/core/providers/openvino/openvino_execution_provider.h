...
static void print_build_options() {
  ...
  std::cout << "are ['CPU','GPU','MYRIAD','FPGA','HDDL','GNA']" << std::endl;
...
static std::vector<std::string> parseDevices(const std::string& device_string) {
  ...
  std::vector<std::string> dev_options = {"CPU", "GPU", "MYRIAD", "FPGA", "HDDL", "GNA"};
...
struct OpenVINOExecutionProviderInfo {
  ...
  explicit OpenVINOExecutionProviderInfo(std::string dev_type, bool enable_vpu_fast_compile, std::string dev_id, size_t num_of_threads, bool use_compiled_network, std::string blob_dump_path)
    ...
    if (dev_type == "") {
      ...
      device_type_ = "MYRIAD";
      precision_ = "FP16";
#elif defined OPENVINO_CONFIG_GNA
      device_type_ = "GNA";
      precision_ = "FP32";
